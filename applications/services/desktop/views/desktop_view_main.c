    ...
    if(main_view->dummy_mode == false) {
        if(event->type == InputTypeShort) {
            ...
            if(event->key == InputKeyUp) {
                ...
                /* The scene owns the configurable Hold-OK action. Keeping the
                 * view policy-free also lets Control Center override the debug
                 * animation shortcut when the user explicitly selects it. */
