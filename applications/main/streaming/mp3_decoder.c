...
static int32_t decoder_task(void* ctx) {
    ...
    while(d->playing) {
        ...
        /* Bad/resynchronised frames must not reach clock configuration or
         * the divisions below, even when the decoder reports success. */
        if(fi.samprate < 8000 || fi.samprate > 48000 ||
           (fi.nChans != 1 && fi.nChans != 2) || fi.outputSamps <= 0 ||
           fi.outputSamps > 2304 || fi.outputSamps % fi.nChans != 0) {
            FURI_LOG_W(TAG, "Discarding invalid MP3 frame metadata");
            continue;
        }
