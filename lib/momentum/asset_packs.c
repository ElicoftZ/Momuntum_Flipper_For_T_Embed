...
static void free_icon(const Icon* icon) {
    ...
    // StaticIconSwap and AnimatedIconSwap have similar structure, but the
    // animated one has a frames array of variable length and holds its frame
    // data in separate allocations, while the static one keeps its single
    // frame in the same allocation. If the first frame points later into this
    // same allocation, it is static.
...
void asset_packs_init(void) {
    ...
    const char* pack = momentum_settings.asset_pack;
    if(pack[0] == '\0') return;
    if(!momentum_settings_asset_pack_is_safe(pack)) return;
    ...
    if(storage_common_stat(storage, furi_string_get_cstr(p), &info) == FSE_OK &&
       info.flags & FSF_DIRECTORY) {
        ...
        memset(asset_packs->fonts, 0, sizeof(asset_packs->fonts));
        memset(asset_packs->font_params, 0, sizeof(asset_packs->font_params));
        ...
        FURI_LOG_I(TAG, "Loaded pack '%s'", pack);
    }
    furi_string_free(p);
...
const Icon* asset_packs_swap_icon(const Icon* requested) {
    if(!asset_packs || !requested) return requested;
    /* Upstream first rejects pointers outside the STM32's flash window, which
     * cheaply skips icons that are themselves replacements. There is no
     * equivalent constant here, and the identity comparison below is already
     * correct for those, so the check is simply omitted. */
    for
        M_EACH(icon_swap, asset_packs->icons, IconSwapList_t) {
...
const uint8_t* asset_packs_swap_font(Font font) {
    if(!asset_packs || font >= FontTotalNumber) return NULL;
    return asset_packs->fonts[font];
}
