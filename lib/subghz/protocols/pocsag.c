...
/* FuriString arrives via pocsag.h -> pcsg_generic.h -> blocks/generic_i.h;
 * the direct <furi/core/string.h> path is not on this port's include dirs. */
...
void* subghz_protocol_decoder_pocsag_alloc(SubGhzEnvironment* environment) {
    ...
    /* The decoder owns four FuriString pointers and reset/free always use all
     * of them. A plain malloc left generic.result_* as indeterminate values,
     * so the NULL checks below could skip their allocation and the first
     * receiver reset would dereference garbage. */
    SubGhzProtocolDecoderPocsag* instance = calloc(1, sizeof(SubGhzProtocolDecoderPocsag));
    ...
    subghz_protocol_decoder_pocsag_reset(instance);
...
uint8_t subghz_protocol_decoder_pocsag_get_hash_data(void* context) {
    ...
    size_t len = furi_string_size(instance->done_msg);
    for(size_t i = 0; i < len; i++)
        hash.split[i % sizeof(hash)] ^= furi_string_get_char(instance->done_msg, i);
    /* The port's decoder interface predates get_hash_data_long: the hash is a
     * uint8_t here, so the 32-bit value is folded down rather than truncated,
     * which would throw away three quarters of the entropy. */
    return (uint8_t)(hash.full ^ (hash.full >> 8) ^ (hash.full >> 16) ^ (hash.full >> 24));
}
