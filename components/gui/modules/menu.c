...
#include "../canvas_i.h" // canvas_set_orientation, for the Vertical style
#include "../elements.h"
...
static void menu_draw_callback(Canvas* canvas, void* _model) {
    ...
    if(items_count) {
        ...
        case MenuStylePs4: {
            ...
            snprintf(str, sizeof(str), "Level %u", (unsigned)stats.level);
        ...
        case MenuStyleC64: {
            ...
            char memstr[32];
            snprintf(
                memstr, sizeof(memstr), "%u BASIC BYTES FREE", (unsigned)memmgr_get_free_heap());
            ...
            for(size_t i = 0; i < 2; i++) {
                for(size_t j = 0; j < 5; j++) {
                    ...
                    char indexstr[12];
                    snprintf(indexstr, sizeof(indexstr), "%u.", (unsigned)index);
        ...
        case MenuStyleMNTM: {
            ...
            canvas_set_font(canvas, FontSecondary);
            char clk[24];
            snprintf(clk, sizeof(clk), "%02u:%02u", (unsigned)hour, (unsigned)min);
            ...
            char bat_display[20];
            snprintf(bat_display, sizeof(bat_display), "%u%%", (unsigned)battery_percent);
            ...
            endItem = (endItem > items_count) ? items_count : endItem;

            for(size_t i = position; i < endItem; i++) {
                item = MenuItemArray_get(model->items, i);
                menu_get_name(item, name, true);
        ...
        case MenuStyleCoverFlow: {
            ...
            for(int8_t i = -3; i <= 3; i++) {
                ...
                if(i < 0) {
                    ...
                    scale_width = scale_base;
                    scale_height = scale_base;
...
/* Takes a position, not a MenuItem index. The loader reuses index values
 * between its internal and external app lists, so searching by index would
 * be ambiguous here. */
        ...
        MenuModel * model,
        {
            ...
            case MenuStyleCoverFlow: {
                ...
                if(position > 0) {
                    ...
                    position = count - 1;
                    vertical_offset = count > 8 ? count - 8 : 0;
                }
    ...
    with_view_model(
        ...
        MenuModel * model,
        {
            ...
            case MenuStyleCoverFlow: {
                ...
                if(position < count - 1) {
                    position++;
                    if(count > 8 && vertical_offset < count - 8 &&
                       position >= 7 && vertical_offset == position - 7) {
                        vertical_offset++;
                    }
                } else {
                    position = 0;
