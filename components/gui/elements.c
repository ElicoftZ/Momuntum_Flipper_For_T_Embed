...
void elements_scrollable_text_line_centered(
    ...
    size_t len_px = canvas_string_width(canvas, furi_string_get_cstr(line));
    if(len_px > width) {
        // A scrolling line already fills the width, so there is nothing left to
        // centre; anchor it left instead. Marquee scrolling keeps the centre,
        // because it returns to a resting position short of the full width.
        if(centered && !momentum_settings.scroll_marquee) {
            centered = false;
            x -= width / 2;
        }
