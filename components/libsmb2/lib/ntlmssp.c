                 ...
                 struct auth_data *auth_data, char *server_challenge)
{
        ...
        /* ESP32 port: our RTC is unset (~1970), so the client timestamp is far
         * in the past and strict servers (macOS) reject it. If the challenge's
         * target info carries a server timestamp (MsvAvTimestamp, AvId=7), use
         * that as the NTLMv2 temp timestamp -- the MS-NLMP-correct behaviour. */
        {
                uint8_t *ti = (uint8_t *)server_name_buf;
                uint32_t off = 0;
                while (off + 4 <= server_name_len) {
                        uint16_t av_id, av_len;
                        memcpy(&av_id, ti + off, 2);
                        av_id = le16toh(av_id);
                        memcpy(&av_len, ti + off + 2, 2);
                        av_len = le16toh(av_len);
                        if (av_id == 0) break; /* MsvAvEOL */
                        if (av_id == 7 && av_len == 8 &&
                            off + 12 <= server_name_len) {
                                uint64_t sts;
                                memcpy(&sts, ti + off + 4, 8);
                                t = le64toh(sts);
                                break;
                        }
                        off += 4 + av_len;
