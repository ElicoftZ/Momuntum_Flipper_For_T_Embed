...
ssize_t writev(t_socket fd, const struct iovec* vector, int count)
{
	...
	ssize_t bytes_written;
        ...
        /* ESP32 port: the fd is a socket; on lwIP <lwip/sockets.h> maps
         * read/write to the generic IO names that take a raw lwIP index, not
         * the VFS fd -> EBADF. send/recv operate on the socket fd directly. */
        bytes_written = send((int)fd, buffer, bytes, 0);
        free(buffer);
...
ssize_t readv(t_socket fd, const struct iovec* vector, int count)
{
        ...
        /* Read the data. ESP32 port: recv on the socket fd (see writev note). */
        bytes_read = recv((int)fd, buffer, bytes, 0);
        if (bytes_read < 0) {
                free(buffer);
                return -1;
        }
