...
#include "../arm_fap/arm_fap_runtime.h"
...
/* Flipper FAP manifests use small STM32 stacks. ESP32 storage, GUI, and
 * filesystem call chains need more room for SD-backed applications. */
#define FAP_ESP32_MIN_STACK_SIZE (12U * 1024U)
#endif
...
static FlipperApplicationPreloadStatus
    ...
    FURI_LOG_D(TAG, "Loading FAP: %s (full=%d)", path, load_full);

    if(!elf_file_open(app->elf, path)) {
        if(!elf_file_is_arm(app->elf)) return FlipperApplicationPreloadStatusInvalidFile;
        return arm_fap_runtime_preload(
            app->storage, path, &app->manifest, load_full, &app->arm);
    }

    // if we are loading full file
    if(load_full) {
        FURI_LOG_D(TAG, "Loading section table...");
        ...
        FURI_LOG_D(TAG, "Section table loaded OK");
    }

    // load manifest section
    FURI_LOG_D(TAG, "Looking for .fapmeta section...");
    ...
    FURI_LOG_D(
        TAG,
        "Manifest: magic=0x%08lX ver=%lu api=%u.%u target=%u stack=%u name='%.32s'",
...
FlipperApplicationLoadStatus flipper_application_map_to_memory(FlipperApplication* app) {
    ...
    if(app->arm) return arm_fap_runtime_map(app->arm);
...
static int32_t flipper_application_thread(void* context) {
    ...
    if(app->arm) return arm_fap_runtime_run(app->arm);
...
FuriThread* flipper_application_alloc_thread(FlipperApplication* app, const char* args) {
    ...
    uint32_t stack_size = manifest->stack_size;
#if defined(CONFIG_IDF_TARGET_ESP32S3) || defined(CONFIG_IDF_TARGET_ESP32S2)
    if(stack_size < FAP_ESP32_MIN_STACK_SIZE) stack_size = FAP_ESP32_MIN_STACK_SIZE;
#endif
    app->thread = furi_thread_alloc_ex_foreground(
        manifest->name, app->arm ? 16384 : stack_size, flipper_application_thread, app);
...
const char* flipper_application_get_load_error(const FlipperApplication* app) {
    return app->arm ? arm_fap_runtime_error(app->arm) : NULL;
}
