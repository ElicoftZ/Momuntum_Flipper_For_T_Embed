...
static const MbEntry system_entries[] = {
    ...
    {0x50aa, 0, 0, 0x20000, 0x400000, "factory", 0},
...
bool mb_layout_valid(const MbEntry* e, size_t count) {
    ...
    for(size_t i = 0; i < count; ++i) {
        ...
        if(system) continue;
        if(e[i].type != 0 || e[i].subtype < 0x10 || e[i].subtype > 0x1f ||
        ...
        const unsigned bit = 1U << (e[i].subtype - 0x10);
        if(subtype_mask & bit) return false;
        subtype_mask |= bit;
        ++apps;
    }
    return systems == 0xff && subtype_mask == ((1U << apps) - 1U);
}
...
bool mb_layout_add(MbEntry* e, size_t* count, uint32_t image_size, MbEntry* added) {
    ...
    for(size_t i = 0; i < *count; ++i) {
        ...
        if(e[i].offset - next >= size) {
            MbEntry entry = {0x50aa, 0, (uint8_t)(0x10 + *count - MB_SYSTEM_COUNT), next, size, {0}, 0};
            snprintf(entry.label, sizeof(entry.label), "fw%u", entry.subtype - 0x10);
...
bool mb_layout_remove(MbEntry* e, size_t* count, uint32_t address) {
    ...
    for(size_t i = 0; i < *count; ++i) {
        if(e[i].offset != address || e[i].type != 0 || e[i].subtype < 0x10) continue;
        memmove(&e[i], &e[i + 1], (*count - i - 1) * sizeof(*e));
        --*count;
        unsigned subtype = 0x10;
        for(size_t j = 0; j < *count; ++j) {
            if(e[j].type == 0 && e[j].subtype >= 0x10) {
                e[j].subtype = (uint8_t)subtype++;
                snprintf(e[j].label, sizeof(e[j].label), "fw%u", e[j].subtype - 0x10);
            }
        }
        return true;
    }
